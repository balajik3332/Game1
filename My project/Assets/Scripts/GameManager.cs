...
public class GameManager : MonoBehaviour
{
    ...
    public void create_enemy(int y)
    {
        int x = 0;
        while (x < y)
        {
            System.Random r = new System.Random();
            // GameObject enemyinfo = Resources.LoadAssetAtPath("Assets/prefab/Enemy.prefab", typeof(GameObject)).GetComponent();
            // GameObject enemyinfo = Resources.LoadAssetAtPath("Assets/prefab/Enemy.prefab", typeof(GameObject));
            // GameObject enemyinfo = Resources.LoadAssetAtPath<GameObject>("Assets/Prefabs/Enemy.prefab");
            // GameObject enemyinfo = Resources.Load("Assets/Prefabs/Enemy.prefab", typeof(GameObject));
            Vector3 enemyPos;
            enemyPos = transform.position;
            enemyPos.y = r.Next(-8, 8);
            enemyPos.x = 15;
            Debug.Log(enemyPos);
            _enemy = Instantiate(enemyinfo, enemyPos, transform.rotation) as GameObject;
            x += 1;
        }
    }
